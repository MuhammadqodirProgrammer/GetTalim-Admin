...
export default function Student() {
    ...
    if(evt.target?.value){
      const res = await instance.get(`api/students/search-full-name/${evt.target?.value}?page=${activePage}`);
      console.log(res?.data, 'student');
    ...
    if(evt.target?.value){
      const res = await instance.get(`api/students/search-email/${evt.target?.value}?page=${activePage}`);
      console.log(res?.data, 'student');
	...
	const editMentorFunc = async (e: any) => {
		...
		formData.append('phoneNumber', editPhoneRef?.current?.value || oneData?.phoneNumber);
		formData.append('isMale', editMaleRef?.current?.value || oneData?.isMale);

		let response = await instance.put(`api/admin/student/${oneData?.id}`, formData);
...
console.log(deleteID ,"deleteID");
	...
	return (
		<div>
			<nav className='flex justify-between mb-3 items-center ' aria-label='Breadcrumb'>
			...
			<div className='flex flex-wrap gap-3 md:justify-start justify-center'>
                ...
                <div className='flex gap-2 mt-2 text-grey'>
								...
								<div className='flex flex-col gap-2 mt-2'>
									...
									<button className='p-2 rounded-md bg-[red] text-white' onClick={()=>{
                    setdeleteModal(true);
										setDeleteID(el?.id);
                  }} >
										Delete
